      ...
      fetch(SETMATE_WEBHOOK, {
        ...
        if (res.ok) {
          ...
          form.innerHTML = '<div style="text-align:center;padding:2rem 1rem;">'
            ...
            + '<p style="color:#555;">Need immediate help? Call <a href="tel:+14254397700" style="color:#d35400;font-weight:700;">(425) 439-7700</a></p>'
            + '</div>';
        } else {
          res.json().then(function(d) {
            alert('Something went wrong: ' + (d.error || 'Please try again.'));
          }).catch(function() { alert('Something went wrong. Please call us at (425) 439-7700.'); });
          if (btn) { btn.disabled = false; btn.textContent = originalText; }
        }
      }).catch(function() {
        alert('Network error. Please call us at (425) 439-7700.');
        if (btn) { btn.disabled = false; btn.textContent = originalText; }
      });
    });
  });
